...
<table class="table">
  ...
  <tbody>
    <tr v-for="(i,index) in carts" :key="index">
      ...
      <td>{{ i.price }} บาท</td>
      <td>
        <div class=" me-2" role="group" aria-label="Second group">
          <button type="button" class="btn btn-outline-primary" @click="plus(index)"><i class="fa-solid fa-plus" style="color: blue;"></i></button>
          <div class="btn btn-light">{{ i.count }}</div>
          <button type="button" class="btn btn-outline-danger" @click="minus(index)" :disabled="i.count === 0"><i class="fa-solid fa-minus" style="color: black;"></i></button>
        </div></td>
...
<div class="text-end float-end m-3 ">
  <div  style="width: 18rem; margin-right: 50px;">
    <div class="card-body">
